...
int main(void)
{
    ...
    double dx,dy,xmax,ymax,rho,mu,omegau,omegav,omegap,total,source_sum=0.0;
    ...
    while(t<=outeriter)
    {
        ...
        for(i=2;i<=grid;i++){
            for(j=1;j<=grid;j++){
                ae[i][j] = std::max(- rho * dy * 0.5 * (uold[i+1][j] + uold[i][j]), 0.0) + mu*(dy/dx);
                aw[i][j] = std::max(rho * dy * 0.5 * (uold[i][j] + uold[i-1][j]), 0.0) + mu*(dy/dx);
                an[i][j] = std::max(-rho * dx * 0.5 * (vold[i][j+1] + vold[i-1][j+1]), 0.0) + mu*(dx/dy);
                as[i][j] = std::max(rho * dx * 0.5 * (vold[i][j] + vold[i-1][j]), 0.0) + mu*(dx/dy);
        ...
        for(i=1;i<=grid;i++){
            for(j=2;j<=grid;j++){
                ae[i][j] = std::max(- rho * dy * 0.5 * (uold[i+1][j] + uold[i+1][j-1]), 0.0) + mu*(dy/dx);
                aw[i][j] = std::max(rho * dy * 0.5 * (uold[i][j] + uold[i][j-1]), 0.0) + mu*(dy/dx);
                an[i][j] = std::max(-rho * dx * 0.5 * (vold[i][j] + vold[i][j+1]), 0.0) + mu*(dx/dy);
                as[i][j] = std::max(rho * dx * 0.5 * (vold[i][j] + vold[i][j-1]), 0.0) + mu*(dx/dy);
        ...
        //std::cout<<"Iteration No:"<<t<<"\t"<<"Mass imbalance after correction:"<<total<<std::endl;
